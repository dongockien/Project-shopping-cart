...
func (uh *UserHandler) GetAllUsers(ctx *gin.Context) {
	...
	utils.ResponseSuccess(ctx, http.StatusOK, "User list succcessfully", paginationResp)
}
...
func (uh *UserHandler) GetUserSortDeleted(ctx *gin.Context) {
	var params v1dto.GetUsersParams
	if err := ctx.ShouldBindQuery(&params); err != nil {
		utils.ResponseValidator(ctx, validation.HandleValidationErrors(err))
		return
	...
	paginationResp := utils.NewPaginationResponse(usersDTO, params.Page, params.Limit, total)
	utils.ResponseSuccess(ctx, http.StatusOK, "List user sort deleted succcessfully", paginationResp)
}
...
func (uh *UserHandler) SoftDeleteUser(ctx *gin.Context) {
	...
	softDeleteUser, err := uh.service.SortDeleteUser(ctx, userUuid)
