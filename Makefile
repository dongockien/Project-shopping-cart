...
# Force migration version(use with cautuon example: make migrate-force VERSION=1)
migrate-force:
...
# Apply specific migration (make migrate-goto VERSION=1)
migrate-goto:
...
print-conn:
	@echo $(CONN_STRING)
.PHONY: importdb exportdb server migrate-create migrate-up migrate-force migrate-drop migrate-goto migrate-down-n sqlc build run-binary prod stop-prod logs-prod bash noapp stop-noapp dev stop-dev
